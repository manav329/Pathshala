...
function Sidenav({ color, brand, brandName, routes, ...rest }) {
  ...
  const { miniSidenav, transparentSidenav, whiteSidenav, darkMode, collapsed } = controller;
  ...
  const filteredRoutes = routes.filter((route) => {
    if (route.type === "route") return true; // Always show auth routes
    if (userType === "student") {
      return !route.key.includes("teacher");
    }
    if (userType === "teacher") {
      return !route.key.includes("student");
    }
    return false;
  });
